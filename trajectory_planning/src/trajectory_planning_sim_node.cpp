...
{
public:
  explicit MoveItNode(const rclcpp::NodeOptions &options);
    ...
    executor_(std::make_shared<rclcpp::executors::SingleThreadedExecutor>())
{
  ...
  pose_subscription_ = this->create_subscription<geometry_msgs::msg::Pose>(
    "joint_space", 10, std::bind(&MoveItNode::joint_space_callback, this, _1));

  joint_subscription_ = this->create_subscription<std_msgs::msg::Float32>(
    "target_wrist_joint", 10, std::bind(&MoveItNode::target_wrist_joint_callback, this, _1));

  speed_scale_sub_ = this->create_subscription<std_msgs::msg::Float32>(
    "speed_scale", 10, std::bind(&MoveItNode::set_speed_scale_callback, this, _1));

  all_joint_subscription_ = this->create_subscription<sensor_msgs::msg::JointState>(
    "target_joints", 10, std::bind(&MoveItNode::target_joints_callback, this, _1));

  pose_array_subscription_ = this->create_subscription<geometry_msgs::msg::PoseArray>(
    "task_space", 10, std::bind(&MoveItNode::task_space_callback, this, _1));

  request_current_pose_subscription_ = this->create_subscription<std_msgs::msg::Bool>(
    "ask_current_pose", 10, std::bind(&MoveItNode::publish_pose, this, _1));
...
void MoveItNode::addCollisionObjects(moveit::planning_interface::PlanningSceneInterface& planning_scene_interface) {
    RCLCPP_INFO(this->get_logger(), "Adding objects");
    // Define the table
...
void MoveItNode::set_speed_scale_callback(const std_msgs::msg::Float32::SharedPtr msg)
{
    speed_scale = msg->data;
    RCLCPP_INFO(this->get_logger(), "Recieved new Speed scale %f", speed_scale);
}

void MoveItNode::target_wrist_joint_callback(const std_msgs::msg::Float32::SharedPtr msg)
{
...
void MoveItNode::target_joints_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
    int i = 0;
...
void MoveItNode::joint_space_callback(const geometry_msgs::msg::Pose::SharedPtr pose_)
{    
    int i = 0;
...
void MoveItNode::publish_pose(const std_msgs::msg::Bool::SharedPtr msg)
...
void MoveItNode::task_space_callback(const geometry_msgs::msg::PoseArray::SharedPtr waypoint_array_msg)
{
...
int main(int argc, char *argv[])
{
  ...
  // Create and spin the MoveItNode
  auto move_group_node = std::make_shared<MoveItNode>(node_options);
  // Check if the parameter "use_sim_time" exists, if not, declare and set it
  if (!move_group_node->has_parameter("use_sim_time"))
  {
    RCLCPP_INFO(logger, "Parameter 'use_sim_time' not found, setting it to true");
    move_group_node->declare_parameter("use_sim_time", true);
    move_group_node->set_parameter(rclcpp::Parameter("use_sim_time", true));
  }

  // Log an example info message
